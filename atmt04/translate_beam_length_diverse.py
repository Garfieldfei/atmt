...
def get_args():
    ...
    # Add data arguments
    parser.add_argument('--data', default='./', help='path to data directory')
    ...
    parser.add_argument('--alpha', default=0.2, type=float, help='length-normalization')
    parser.add_argument('--gamma', default=0.8, type=float, help='diverse-beam-search')
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        batch_size = sample['src_tokens'].shape[0] # batch_size = 1
        # print(sample['src_tokens'].shape[1]) # input sentence length in ascending order
        ...
        for _ in range(args.max_len-1):
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    length_normal = ((5 + next_word.shape[0]) / 6.0) ** args.alpha
                    log_p = torch.div(log_p, length_normal)

                    # disverse beam search
                    log_p = log_p - (j+1)*args.gamma
        ...
        for search in searches:
            get_n_best = search.get_best()
            for n in range(len(get_n_best)):
                best_sents = torch.stack([get_n_best[n][1].sequence[1:]])
                ...
                for ii, sent in enumerate(output_sentences):
                    all_hyps[n][int(sample['id'].data[ii])] = sent


    for i in range(len(all_hyps)):
    # Write to file
        if args.output is not None:
            with open(args.output + str(i+1) + ".txt", 'w') as out_file:
                for sent_id in range(len(all_hyps[i].keys())):
                    out_file.write(all_hyps[i][sent_id] + '\n')
